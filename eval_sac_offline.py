...
from common.train_offline_utils import normalize_obs as common_normalize_obs
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Eval SAC Agent Offline")
    ...
    model_path = os.path.join(experiment_dir, "model", args.model_filename)
    ...
    obs_mean, obs_std = get_normalization_stats(results_dir=experiment_dir)
    ...
    for idx, config_path in enumerate(config_paths):
        ...
        while not done:
            normalized_obs = common_normalize_obs(obs, mean=obs_mean, std=obs_std)
    ...
    logger.info(
        f"Mean Load Balance: {mean_load_balance:.4f} +/- {std_load_balance:.4f}"
    )
    logger.info("-" * 50)
    ...
    output_file = os.path.join(experiment_dir, "evaluation_results.json")
