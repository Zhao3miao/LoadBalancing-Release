...
class TransitionModel:
    ...
    def predict(self, obs, act, deterministic=False):
        ...
        # Explicitly construct samples to avoid numpy advanced indexing issues
        pred_diff_samples = np.zeros((batch_size, pred_diff_means.shape[2]))
        for i in range(batch_size):
            pred_diff_samples[i] = pred_diff_means[model_idxes[i], i]

        rewards = pred_diff_samples[:, -1:]
        next_obs = pred_diff_samples[:, :-1] + obs
        
        # Ensure rewards shape (B, 1)
        if rewards.ndim == 1:
