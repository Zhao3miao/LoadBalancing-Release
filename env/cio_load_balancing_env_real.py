...
try:
    ...
    print(
        f"Warning: Unable to import OSM-related libraries, falling back to simplified mode: {e}"
    )
    OSM_AVAILABLE = False
...
class CIOLoadBalancingEnv(gym.Env):
    def __init__(self, config):
        ...
        # State space: only includes RSRP values (consistent with CIOLoadBalancingEnv_with_RSRP)
        self.num_base_stations = len(self.base_stations)
        self.num_devices = len(self.mobile_devices)
        self.state_dim = self.num_base_stations * self.num_devices
        ...
        self.osm_file_path = self.config.get("osm_file_path", "load_balancing/map.osm")
        self.osm_graph = None
        ...
        self.average_load_history = deque(maxlen=100)
    ...
    def _load_osm_map(self):
        """Load OSM map data and convert to metric coordinate system"""
        if self.map_loaded:
            return
        ...
        try:
            ...
            self._convert_osm_to_meters()

            self.map_loaded = True
            print(
                f"OSM map loaded: {len(self.road_nodes)} road nodes, {len(self.road_edges)} road edges"
            )
        except Exception as e:
            print(f"Error loading OSM map: {e}")
            self.map_loaded = False

    def _convert_osm_to_meters(self):
        """Convert OSM map latitude/longitude coordinates to metric coordinate system"""
        min_lon = self.road_nodes.geometry.x.min()
        min_lat = self.road_nodes.geometry.y.min()
        max_lon = self.road_nodes.geometry.x.max()
        max_lat = self.road_nodes.geometry.y.max()
        ...
        for node_id, node_data in self.road_nodes.iterrows():
            ...
            x_meters = lon_to_meters(lon)
            y_meters = lat_to_meters(lat)
            from shapely.geometry import Point

            self.road_nodes.at[node_id, "geometry"] = Point(x_meters, y_meters)

        for edge_id, edge_data in self.road_edges.iterrows():
            if hasattr(edge_data.geometry, "coords"):
                ...
                for lon, lat in coords:
                    x_meters = lon_to_meters(lon)
                    y_meters = lat_to_meters(lat)
                    converted_coords.append((x_meters, y_meters))
                from shapely.geometry import LineString

                self.road_edges.at[edge_id, "geometry"] = LineString(converted_coords)

        print("OSM map coordinates converted to metric coordinate system")
    ...
    def render(self, mode="human"):
        ...
        if self.map_loaded:
            for _, edge in self.road_edges.iterrows():
                if hasattr(edge.geometry, "coords"):
                    coords = list(edge.geometry.coords)
                    x_coords = [coord[0] for coord in coords]
                    y_coords = [coord[1] for coord in coords]
                    self.ax.plot(x_coords, y_coords, "gray", linewidth=0.5, alpha=0.7)
        min_x = 0
        ...
        self.ax.set_title(
            f"Real Scenario CIO Load Balancing - Step: {self.current_time}", fontsize=16
        )
        ...
        load_info = "Loads:\n"
        for bs in self.base_stations:
            load_percentage = bs["resource_utilization"]
            num_devices = len(bs["connected_devices"])
            load_info += f"BS{bs['id']}: {load_percentage:.2f} ({num_devices} users)\n"
